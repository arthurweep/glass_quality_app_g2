...
def bayes_opt_adjustment(clf, x_orig, feature_names, target_prob=0.5):
    ...
    def objective(**kwargs):
        ...
        if prob >= target_prob:
            ...
            return penalty + (target_prob - prob) * 10
    res = gp_minimize(objective, dims, acq_func='EI', n_calls=30, n_random_starts=10)
    best_adj = res.x
    best_prob = clf.predict_proba(pd.DataFrame([np.array(x_orig)+np.array(best_adj)], columns=feature_names))[0,1]
    return best_adj, best_prob
...
def predict_single_ajax():
    ...
    try:
        ...
        if prob_ok < threshold:
            ...
            best_adj, best_prob = bayes_opt_adjustment(clf, x_orig, feature_names, target_prob=threshold)
            suggestion_lines = []
