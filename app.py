...
def bayes_opt_adjustment(clf, x_orig, feature_names):
    """
    贝叶斯优化寻找最小调整使预测概率达到或超过阈值（这里不固定阈值，目标是最大化概率）
    """
    ...
    def objective(**kwargs):
        ...
        # 目标是最大化概率，gp_minimize最小化目标函数，故取负
        return -prob
    ...
    if request.method == 'POST':
        ...
        if file and file.filename.endswith('.csv'):
            try:
                ...
                model_cache['recommended_threshold_text'] = f"ℹ️ 分类阈值固定为: {fixed_threshold:.1f}"
